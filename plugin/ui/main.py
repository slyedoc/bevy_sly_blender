...
## components ui
def draw_invalid_or_unregistered(layout, components_registry, components_list, status, component_name, object):
...
class BEVY_PT_SidePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        tool_switch_components = target.operator(OT_switch_bevy_tooling.bl_idname, text="", icon="PROPERTIES")
        tool_switch_components.tool = "COMPONENTS"

        target = row.box() if bevy.mode == 'BLUEPRINTS' else row
        tool_switch_components = target.operator(OT_switch_bevy_tooling.bl_idname, text="", icon="PACKAGE")
        tool_switch_components.tool = "BLUEPRINTS"

        target = row.box() if bevy.mode == 'ASSETS' else row
        tool_switch_components = target.operator(OT_switch_bevy_tooling.bl_idname, text="", icon="ASSET_MANAGER")
        tool_switch_components.tool = "ASSETS"

        target = row.box() if bevy.mode == 'SETTINGS' else row
        tool_switch_components = target.operator(OT_switch_bevy_tooling.bl_idname, text="", icon="SETTINGS")
        tool_switch_components.tool = "SETTINGS"

        target = row.box() if bevy.mode == 'TOOLS' else row
        tool_switch_components = target.operator(OT_switch_bevy_tooling.bl_idname, text="", icon="TOOL_SETTINGS")
